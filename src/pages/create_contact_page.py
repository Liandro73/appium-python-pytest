...
class CreateContactPage(PageObjectHelper):
    ...
    def fill_input_first_name(cls, driver, os, firstname: str, wait):
        if os == 'ios':
            input_first_name: WebElement = driver.find_element(*cls.input_first_name_ios)
            cls.clear_and_type_in_element(wait, input_first_name, firstname)
        else:
    ...
    def fill_input_last_name(cls, driver, os, firstname: str, wait):
        if os == 'ios':
            input_last_name: WebElement = driver.find_element(*cls.input_last_name_ios)
            cls.clear_and_type_in_element(wait, input_last_name, firstname)
        else:
    ...
    def fill_input_company(cls, driver, os, company: str, wait):
        if os == 'ios':
            input_company: WebElement = driver.find_element(*cls.input_company_ios)
            cls.clear_and_type_in_element(wait, input_company, company)
        else:
    ...
    def click_on_button_add_phone(cls, driver, os, wait):
        if os == 'ios':
            input_company: WebElement = driver.find_element(*cls.btn_add_phone_ios)
            cls.click_on_element(wait, input_company)

    @classmethod
    @allure.step("Click on button add phone")
    def click_on_button_select_phone_type(cls, driver, os, wait):
        if os == 'ios':
            btn_phone_type: WebElement = driver.find_element(*cls.btn_phone_type_ios)
            cls.click_on_element(wait, btn_phone_type)
        else:
    ...
    def fill_input_phone(cls, driver, os, phone: str, wait):
        cls.click_on_button_add_phone(driver, os, wait)
        if os == 'ios':
            input_phone: WebElement = driver.find_element(*cls.input_phone_ios)
            cls.clear_and_type_in_element(wait, input_phone, phone)
        else:
    ...
    def click_on_button_save_contact(cls, driver, os, wait):
        if os == 'ios':
            btn_save_contact: WebElement = driver.find_element(*cls.btn_save_contact_ios)
            cls.click_on_element(wait, btn_save_contact)
        else:
